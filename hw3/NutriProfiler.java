...
public class NutriProfiler {
	...
	public enum NutriEnum {
        ...
        //create personal nutrient profile based on personal information
        static void createNutriProfile(Person person) {
            //write your code here
            if (person == null) {return;}
            if (person instanceof Female) {
                float[] recommendedNutri = person.calculateNutriRequirement();
                //add energy requirement
                person.recommendedNutrientsList.add(0, new RecommendedNutrient(ENERGY_NUTRIENT_CODE, person.calculateEnergyRequirement()));
                //add all other nutrients
                for (int i = 0; i < RECOMMENDED_NUTRI_COUNT; i++) {
                    for (NutriEnum ne : NutriEnum.values()) {
                        if (ne.getNutriIndex() == i) {
                            person.recommendedNutrientsList.add(i + 1, new RecommendedNutrient(ne.nutrientCode,recommendedNutri[i]));
                        }
                    }
                }
            } else if (person instanceof Male) {
                float[] recommendedNutri = person.calculateNutriRequirement();
                //add energy requirement
                person.recommendedNutrientsList.add(0, new RecommendedNutrient(ENERGY_NUTRIENT_CODE, person.calculateEnergyRequirement()));
                //add all other nutrients
