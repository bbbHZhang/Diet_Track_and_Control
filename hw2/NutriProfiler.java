	...
	public enum NutriEnum {
        ...
        static void createNutriProfile(Person person) {
            ...
            if (person instanceof Female) {
                ...
                recommendedNutrientsList.add(0, new RecommendedNutrient(ENERGY_NUTRIENT_CODE, person.calculateEnergyRequirement()));
                //add all other nutrients
                for (int i = 0; i < RECOMMENDED_NUTRI_COUNT; i++) {
                    for (NutriEnum ne : NutriEnum.values()) {
                        if (ne.getNutriIndex() == i) {
                            recommendedNutrientsList.add(i + 1, new RecommendedNutrient(ne.nutrientCode,recommendedNutri[i]));
                        }
                    }
                }
